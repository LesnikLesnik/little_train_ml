...
- Используется вероятно **не самый оптимальный способ подбора гиперпараметров** (тестировался алгоритм случайного подбора и вариант оптимизированного метода).
...
- **Цель модели:** отобрать подмножество студентов с высокой вероятностью наличия депрессии для направления на дальнейшую диагностику.
- **Допущение:** повысить точность выявления (recall) даже за счёт снижения чувствительности (precision).
- **Обоснование:** обогащение выборки — уменьшение объема обследуемых из всей совокупности до более узкой выборки при сохранении подавляющего числа действительно больных внутри обогащенной выборки.
...
### 2 сценария валидации (группы тестирования):

#### Сценарий 1: Пессимистичный (60% больных)
 Предполагается что в среде где применяется модель - больных депрессией порядка 60%. Желаемые показатели по классу "депрессия" в этом случае:
- **Recall**: ≥ 90%
- **Precision**: ≥ 75%

#### Сценарий 2: Оптимистичный (10% больных)
  Предполагается что в среде где применяется модель - больных депрессией порядка 10%. Желаемые показатели по классу "депрессия" в этом случае:
- **Recall**: ≥ 90%
- **Precision**: ≥ 20%
...
- **Precision (депрессия)** = **75%**  
  → среди студентов, предсказанных моделью как депрессивные, в 75% случаев диагноз действительно должен быть подтверждён. 25% предсказаний - ложно-положительные
